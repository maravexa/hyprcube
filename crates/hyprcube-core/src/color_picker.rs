...
// Layout constants (all in logical pixels = physical pixels for now)
...
impl InlineColorPicker {
    ...
    /// Recompute `self.value` from current HSV + stored alpha, then sync hex_input.
    ...
    /// Try to apply hex_input to value; returns true on success.
    ...
    // -----------------------------------------------------------------------
    // Sub-region rects (in pixmap space, given the widget's top-left bounds)
    // -----------------------------------------------------------------------
    ...
    fn paint_sv_square(&self, pixmap: &mut PixmapMut<'_>, r: &Rect) {
        ...
        let white = tiny_skia::Color::from_rgba(1.0, 1.0, 1.0, 1.0).unwrap_or(tiny_skia::Color::WHITE);
        let black_t = tiny_skia::Color::from_rgba(0.0, 0.0, 0.0, 0.0).unwrap_or(tiny_skia::Color::BLACK);
        let black_o = tiny_skia::Color::from_rgba(0.0, 0.0, 0.0, 1.0).unwrap_or(tiny_skia::Color::BLACK);
        ...
        // Pass 1: horizontal gradient white → hue_color
        ...
        // Pass 2: vertical overlay transparent → black (darkens toward bottom)
        if let Some(shader) = LinearGradient::new(
            tiny_skia::Point::from_xy(r.x, r.y),
            tiny_skia::Point::from_xy(r.x, r.y + r.height),
            vec![GradientStop::new(0.0, black_t), GradientStop::new(1.0, black_o)],
            SpreadMode::Pad,
            Transform::identity(),
        ) {
            let mut paint = Paint::default();
        ...
        let cr = 5.0;
        // Draw white outer ring then dark inner ring
    ...
    fn paint_hue_bar(&self, pixmap: &mut PixmapMut<'_>, r: &Rect) {
        ...
        fill_rounded_rect(pixmap, ix - 1.0, r.y - 2.0, 2.0, r.height + 4.0, 1.0, Color::new(1.0, 1.0, 1.0, 1.0));
    }
    ...
    fn paint_alpha_bar(&self, pixmap: &mut PixmapMut<'_>, r: &Rect) {
        ...
        // Alpha gradient: transparent color → opaque color
        let skia_rect = match tiny_skia::Rect::from_xywh(r.x, r.y, r.width, r.height) {
            Some(rect) => rect,
            None => return,
        };
        ...
        if let Some(shader) = LinearGradient::new(
            tiny_skia::Point::from_xy(r.x, r.y),
            tiny_skia::Point::from_xy(r.x + r.width, r.y),
            vec![GradientStop::new(0.0, color_t), GradientStop::new(1.0, color_o)],
            SpreadMode::Pad,
            Transform::identity(),
        ) {
            let mut paint = Paint::default();
...
fn draw_circle_outline(pixmap: &mut PixmapMut<'_>, cx: f32, cy: f32, r: f32, color: Color) {
    ...
    // Approximate circle with cubic bezier segments
    let k = 0.5522847_f32; // control point distance for unit circle approximation
    pb.move_to(cx + r, cy);
...
impl Widget for InlineColorPicker {
    ...
    fn paint(&self, pixmap: &mut PixmapMut<'_>, bounds: Rect, ctx: &mut RenderContext<'_>) {
        ...
        let header = Rect { x: bounds.x, y: bounds.y, width: bounds.width, height: COLLAPSED_H };

        fill_rounded_rect(pixmap, header.x, header.y, header.width, header.height, 6.0, surface);
        ...
        ctx.text.draw_text(pixmap, &hex, text_x, text_y, font_size, fg, header.width - 60.0);

        // Chevron (▼ or ▲)
        let chev_cx = header.x + header.width - 14.0;
        let chev_cy = header.y + COLLAPSED_H / 2.0;
        draw_chevron(pixmap, chev_cx, chev_cy, overlay, self.expanded);
        ...
        // Hue bar
        fill_rounded_rect(pixmap, hue_r.x, hue_r.y, hue_r.width, hue_r.height, 4.0, overlay);
        self.paint_hue_bar(pixmap, &hue_r);

        // Alpha bar
        fill_rounded_rect(pixmap, alpha_r.x, alpha_r.y, alpha_r.width, alpha_r.height, 4.0, overlay);
        self.paint_alpha_bar(pixmap, &alpha_r);

        // Hex input row
        let hex_input_w = hex_r.width - 64.0; // leave room for Apply button
        let hex_border = if self.hex_focused { accent } else { overlay };

        fill_rounded_rect(pixmap, hex_r.x, hex_r.y, hex_input_w, hex_r.height, 6.0, hex_border);
        fill_rounded_rect(
            pixmap,
            hex_r.x + 1.0,
            hex_r.y + 1.0,
        ...
        ctx.text.draw_text(pixmap, &self.hex_input, hex_r.x + 8.0, hex_text_y, font_size, fg, hex_input_w - 16.0);
    ...
    fn event(&mut self, event: &Event, bounds: &Rect) -> EventResponse {
        match event {
            ...
            Event::PointerUp { x, y } => {
                ...
                if header.contains(*x, *y) && self.dragging == DragTarget::None {
                ...
                if self.expanded {
                    // Apply button
                    let hex_r = self.hex_row_rect(bounds);
                    let btn_x = hex_r.x + (hex_r.width - 64.0) + 8.0;
                    let btn_rect = Rect { x: btn_x, y: hex_r.y, width: 54.0, height: hex_r.height };
                    ...
                    let hex_input_rect = Rect {
                        x: hex_r.x,
                        y: hex_r.y,
                        width: hex_r.width - 64.0,
                        height: hex_r.height,
                    };
                    self.hex_focused = hex_input_rect.contains(*x, *y);
                }
