    ...
    pub fn config_line(&self) -> String {
        ...
        if self.transform != 0 {
            format!(
                ...
                r = self.refresh, x = self.x, y = self.y,
                s = self.scale,  t = self.transform,
            )
        } else {
            format!(
...
fn ipc_socket_path() -> Option<std::path::PathBuf> {
    ...
    let path = std::path::PathBuf::from(runtime_dir)
        .join("hypr").join(&instance_sig).join(".socket.sock");
...
impl DisplayPanel {
    ...
    fn load_monitors() -> Vec<MonitorInfo> {
        if let Some(json) = ipc_query_json("monitors") {
    ...
    fn refresh_options(mon: &MonitorInfo, res: &str) -> Vec<String> {
        ...
        let mut rates: Vec<f32> = mon.available_modes.iter()
    ...
    fn rebuild_refresh_for_selected_res(&mut self) {
        ...
        let res = self.resolution_dropdown.options
    ...
    fn apply_selected(&mut self) {
        ...
        let res   = self.resolution_dropdown.options
            .get(self.resolution_dropdown.selected).cloned().unwrap_or_default();
        let ref_s = self.refresh_dropdown.options
            .get(self.refresh_dropdown.selected).cloned().unwrap_or_default();
        ...
        if let Some(mon) = self.monitors.get_mut(idx) {
            ...
            if let Ok(r) = ref_s.parse::<f32>() { mon.refresh   = r; }
            mon.scale     = scale;
    ...
    fn viz_transform(&self, viz: &Rect) -> (f32, f32, f32) {
        ...
        let max_x = self.monitors.iter().map(|m| m.x + m.width  as i32).max().unwrap() as f32;
        let max_y = self.monitors.iter().map(|m| m.y + m.height as i32).max().unwrap() as f32;
...
impl SettingsPanel for DisplayPanel {
    ...
    fn fields(&self) -> Vec<PanelField> { Vec::new() }
    ...
    fn paint(
        ...
        let viz = Rect { x: bounds.x, y: bounds.y, width: bounds.width, height: VIZ_H };
        ...
        paint_dotted_grid(pixmap, viz, Color::new(
            overlay.r, overlay.g, overlay.b, overlay.a * 0.3,
        ));
        ...
        if self.monitors.is_empty() {
            ...
            if let Some(ref drag) = self.drag_state {
                paint_snap_guides(
                    pixmap, drag.monitor_idx, self.drag_live_x, self.drag_live_y,
                    &self.monitors, vs, vox, voy, accent, viz,
                );
            }
            ...
            for (i, mon) in self.monitors.iter().enumerate() {
                ...
                let draw_mon;
                let mon_ref = if self.drag_state.as_ref().map_or(false, |d| d.monitor_idx == i) {
                    draw_mon = MonitorInfo { x: self.drag_live_x, y: self.drag_live_y, ..mon.clone() };
                    &draw_mon
                } else {
                    mon
                };
                ...
                fill_rounded_rect(
                    pixmap,
                    mr.x - 2.0, mr.y - 2.0, mr.width + 4.0, mr.height + 4.0,
                    6.0, border_c,
                );
        ...
        // ── Separator ─────────────────────────────────────────────────────────
        fill_rounded_rect(pixmap, bounds.x, bounds.y + VIZ_H + 4.0, bounds.width, 1.0, 0.0, overlay);
        ...
        ctx.text.draw_text(
            pixmap, &header_text,
            bounds.x + H_PAD, bounds.y + VIZ_H + 8.0,
            font_size + 2.0, fg, bounds.width - H_PAD * 2.0,
        );
        ...
        if self.selected.is_some() {
            ...
            self.resolution_dropdown.paint_overlay(pixmap, self.widget_rect(bounds, 0), ctx);
            self.refresh_dropdown   .paint_overlay(pixmap, self.widget_rect(bounds, 1), ctx);
            self.transform_dropdown .paint_overlay(pixmap, self.widget_rect(bounds, 3), ctx);
        }
    }
...
impl DisplayPanel {
    fn route_form(&mut self, event: &Event, bounds: Rect) -> EventResponse {
        ...
        // Refresh dropdown
        {
            ...
            if resp.consumed { self.apply_selected(); return resp; }
        }
        // Scale slider
        {
...
fn paint_snap_guides(
    ...
    let draw_vline = |pixmap: &mut tiny_skia::PixmapMut<'_>, lx_log: i32| {
        ...
        if sx < viz.x || sx > viz.x + viz.width { return; }
        let mut pb = PathBuilder::new();
        pb.move_to(sx, viz.y);
    ...
    let draw_hline = |pixmap: &mut tiny_skia::PixmapMut<'_>, ly_log: i32| {
        ...
        if sy < viz.y || sy > viz.y + viz.height { return; }
        let mut pb = PathBuilder::new();
        pb.move_to(viz.x, sy);
    ...
    for (i, other) in monitors.iter().enumerate() {
        ...
        let ow = other.width  as i32;
        let oh = other.height as i32;
        if (lx + mw - other.x).abs()      < s { draw_vline(pixmap, other.x); }
        if (lx - (other.x + ow)).abs()    < s { draw_vline(pixmap, other.x + ow); }
        if (lx - other.x).abs()           < s { draw_vline(pixmap, other.x); }
        if (ly + mh - other.y).abs()      < s { draw_hline(pixmap, other.y); }
        if (ly - (other.y + oh)).abs()    < s { draw_hline(pixmap, other.y + oh); }
        if (ly - other.y).abs()           < s { draw_hline(pixmap, other.y); }
    }
}
...
mod tests {
    ...
    fn mon(name: &str, w: u32, h: u32, x: i32, y: i32) -> MonitorInfo {
        MonitorInfo {
            ...
            width: w, height: h, refresh: 60.0,
            x, y, scale: 1.0, transform: 0, enabled: true,
    ...
    fn config_line_disabled() {
        let m = MonitorInfo { enabled: false, ..mon("DP-1", 1920, 1080, 0, 0) };
        assert_eq!(m.config_line(), "monitor = DP-1, disable");
    }

    #[test]
    fn config_line_transform() {
        let m = MonitorInfo { transform: 1, ..mon("DP-1", 1920, 1080, 0, 0) };
    ...
    fn resolution_options_from_modes() {
        ...
        m.available_modes = vec![
            MonitorMode { width: 2560, height: 1440, refresh: 165.0 },
            MonitorMode { width: 1920, height: 1080, refresh: 60.0  },
        ];
        let opts = DisplayPanel::resolution_options(&m);
    ...
    fn refresh_options_filtered_by_resolution() {
        ...
        m.available_modes = vec![
            MonitorMode { width: 2560, height: 1440, refresh: 165.0 },
            MonitorMode { width: 2560, height: 1440, refresh: 60.0  },
            MonitorMode { width: 1920, height: 1080, refresh: 144.0 },
        ];
