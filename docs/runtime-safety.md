...
| `$XDG_CONFIG_HOME/hypr/hyprdeck.toml` | Optional HyprDeck configuration. | Read for panel availability and initial values; panel-local edits are not written by the shared Save pipeline. |
| `$XDG_CONFIG_HOME/hypr/hyprsaver.toml` | Optional HyprSaver configuration. | Read for panel availability and initial values; panel-local edits are not written by the shared Save pipeline. |
...
Display edits follow a separate path. When the Display implementation's custom
event hook is exercised, it sends synchronous `keyword monitor ...` requests
directly to the command socket for monitor enablement, resolution, refresh
rate, scale, and position drags. These requests do not enter `PreviewEngine`'s
undo stack. The panel also tracks a transform selection, but the direct IPC
request currently does not include that transform value. Display edits do not
add monitor lines to the shared `HyprlandConfig`; the ordinary Save action
persists only that shared parsed configuration and the palette. The custom
panel event path is not yet fully wired through the main loop; see
...
5. Clear the normal preview undo stack.

The two file writes are sequential rather than transactional. A later failure
can leave an earlier write in place, and the current handler logs a Save error
but still continues into its reload and undo-stack commit sequence. The backup
also has one fixed name, so each successful pre-save copy replaces the prior
backup.
...
The Save/Revert action bar is driven by the normal preview undo stack. Changes
without a Hyprland section do not make that stack dirty by themselves.
...
It therefore requests elevated privileges and changes `/usr/local/sbin`. Do
